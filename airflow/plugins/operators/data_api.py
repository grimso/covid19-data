...
def fetch_and_extract_covid_data_from_rki():
    ...
    while exceeded_transfer_limit:
        with urllib.request.urlopen(f"https://services7.arcgis.com/mOBPykOjAyBO2ZKk/arcgis/rest/services/RKI_COVID19/FeatureServer/0/query?where=1%3D1&outFields=*&outSR=4326&f=json&resultOffset={offset}") as url:
...
class CovidApiToS3Operator(BaseOperator):
    ...
    def execute(self, context):
        ...
        s3_hook.load_bytes(compressed_value,
...
class AirPollutionApiToS3Operator(BaseOperator):
    ...
    def execute(self, context):
        ...
        pollutant_eval_id=AirPollutionApiToS3Operator.polutant_info[self.pollutant]["eval_id"]
        with urllib.request.urlopen(f"https://www.umweltbundesamt.de/api/air_data/v2/measures/csv?date_from={self.date.strftime('%Y-%m-%d')}&time_from=12&date_to={self.date.strftime('%Y-%m-%d')}&time_to=12&data%5B0%5D%5Bco%5D={pollutant_id}&data%5B0%5D%5Bsc%5D={pollutant_eval_id}&lang=de")as url:
            data = url.read()
            
        s3_hook.load_bytes(gzip.compress(data),
